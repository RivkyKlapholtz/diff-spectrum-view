...
export function LargeResponseViewer({ prodValue, integValue }: LargeResponseViewerProps) {
  ...
  // Very large content - show warning and download option
  if (isVeryLarge && !showFullContent) {
    return (
      <Card>
        <CardHeader>
        ...
        <CardContent className="space-y-4">
          <Alert variant="destructive">
            ...
            <AlertTitle>תגובות גדולות מאוד</AlertTitle>
            <AlertDescription>
              הresponses גדולים מאוד ({formatBytes(totalSize)} ביחד) ועלולים לגרום לבעיות ביצועים.
              מומלץ להוריד את הקבצים ולהשוות אותם בכלי חיצוני.
            </AlertDescription>
          </Alert>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Production Response</span>
                <Badge variant="outline">{formatBytes(prodSize)}</Badge>
              </div>
              <Button 
                variant="outline" 
                className="w-full"
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Integration Response</span>
                <Badge variant="outline">{formatBytes(integSize)}</Badge>
              </div>
              <Button 
                variant="outline" 
                className="w-full"
                onClick={() => downloadResponse(integValue, "integration-response.json")}
              >
                <Download className="h-4 w-4 mr-2" />
                הורד Integration
              </Button>
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              הצג בכל זאת (עלול להאט את הדפדפן)
            </Button>
            <Button 
              variant="outline"
              onClick={() => setViewMode("side-by-side-virtual")}
              className="flex-1"
            >
              הצג עם Virtualization
            </Button>
          </div>

          {viewMode === "side-by-side-virtual" && (
            <VirtualizedSideBySide 
              prodLines={prodLines} 
              integLines={integLines}
